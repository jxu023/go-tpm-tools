...
func getExpectedMeasurement(endorsement []byte) ([]byte, error) {
	launch_endorsement := &epb.VMLaunchEndorsement{}
	err := proto.Unmarshal(endorsement, launch_endorsement)
	if err != nil {
		return nil, fmt.Errorf("failed to unmarshal endorsement: %w", err)
	}
	golden := &epb.VMGoldenMeasurement{}
	err = proto.Unmarshal(launch_endorsement.GetSerializedUefiGolden(), golden)
	if err != nil {
