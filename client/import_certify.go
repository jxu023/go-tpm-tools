...
func makeAK(tpm transport.TPM, keyAlgo tpm2.TPMAlgID) (*tpm2.CreatePrimaryResponse, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("makeAK: = %w", err)
	}
	cp, err := tpm2.CreatePrimary{
...
// SolveChallengeImportCertify solves a challenge from the TPM EK.
func SolveChallengeImportCertify(tpm transport.TPM, req *tpb.ImportBlob, keyAlgo tpm2.TPMAlgID) (*rpb.CertifiedBlob, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("AK() = %w", err)
	}
	defer tpm2.FlushContext{
